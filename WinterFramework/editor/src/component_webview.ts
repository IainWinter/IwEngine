...
export class ComponentPanelProvider implements vscode.WebviewViewProvider
{
	private static exturi : vscode.Uri;
	private static view : vscode.WebviewView;
	
	public static component_state : any;
	...
	public static SetComponentStateForNewEntity(components: string)
	{
		ComponentPanelProvider.component_state = JSONBig.parse(components);
		ComponentPanelProvider.UpdateHtmlForWebview();
	}

	public static SetComponentValue(component_name_tree: string[], value : any, value_type: string)
	{
		...
		switch(value_type)
		{
			case 'number':  current[final_name] = Number(value);  break;
			case 'boolean': current[final_name] = Boolean(value); break;
			default:        current[final_name] = value;          break;
		}
	}

	private static UpdateHtmlForWebview()
	{
		let json_string = "[]";
