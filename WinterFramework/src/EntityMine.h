...
struct entity_storage
{
		...
		entity_storage& new_store)
	{
		...
		int max_length = old_length > new_length ? old_length : new_length; // max(old_length, new_length);
	...
	struct itr
	{
		...
		result_t operator*() const
		{
			if constexpr (_with_entity)
			{
				...
				bool test = !m_current->m_store->is_entity_alive(handle);
