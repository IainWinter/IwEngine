...
#pragma region no_return_no_args
	...
	void foreach_na_indexed(
		_functor&& functor,
		_tuple&& tuple,
		std::index_sequence<m_index...>)
	{
		auto e = {(
			functor(std::get<m_index>(std::forward<_tuple>(tuple))),
			0)...
		};
	}
#pragma endregion
...
#pragma region no_tuple_return_args
		...
		std::index_sequence<m_index...>)
	{
		auto e = { (
			functor(
				std::get<m_index>(std::forward<_tuple>(tuple)), 
				std::get<m_index>(std::forward<_tuple_args>(tuple_args))),
			0)...
		};
	}
...
#pragma region return_tuple_args
		...
		std::index_sequence<m_index...>)
	{
		return {
			functor(
				std::get<m_index>(std::forward<_tuple>(tuple)),
				std::get<m_index>(std::forward(tuple_args)))...
		};
	}
