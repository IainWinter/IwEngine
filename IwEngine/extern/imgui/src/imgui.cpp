...
/*
 ...
 - Dear ImGui never touches or knows about your GPU state. The only function that knows about GPU is the draw function that you provide.
   ...
   phases of your own application. All rendering informatioe are stored into command-lists that you will retrieve after calling ImGui::RenderAPI().
 ...
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the examples/ folder).
     ...
     // Initialize helper Platform and RenderAPI bindings (here we are using imgui_impl_win32 and imgui_impl_dx11)
     ...
     while (true)
     {
         ...
         // RenderAPI dear imgui into screen
         ImGui::RenderAPI();
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE.
     ...
     // (In the examples/ app this is usually done within the ImGui_ImplXXX_Init() function from one of the demo RenderAPI)
     int width, height;
     ...
     while (true)
     {
        ...
        // RenderAPI imgui, swap buffers
        // (You want to try calling EndFrame/RenderAPI as late as you can, to be able to use imgui in your own game rendering code)
        ImGui::EndFrame();
        ImGui::RenderAPI();
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // RenderAPI 'pcmd->ElemCount/3' indexed triangles.
 ...
 - 2018/02/16 (1.60) - obsoleted the io.RenderDrawListsFn callback, you can call your graphics engine render function after ImGui::RenderAPI(). Use ImGui::GetDrawData() to retrieve the ImDrawData* to display.
 ...
 A: Short explanation:
    ...
    The renderer function called after ImGui::RenderAPI() will receive that same value that the user code passed:
 ...
 A: In your RenderAPI function, try translating your projection matrix by (0.5f,0.5f) or (0.375f,0.375f).
 ...
 - tip: you can call RenderAPI() multiple times (e.g for VR renders).
...
// RenderAPI a rectangle shaped with optional rounding and borders
...
// RenderAPI an arrow aimed to be aligned with text (p_min is a position in the same space text would be positioned). To e.g. denote expanded/collapsed state
...
// Same value as passed to the old io.RenderDrawListsFn function. Valid after RenderAPI() and until the next call to NewFrame()
...
void ImGui::NewFrame()
{
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
            IM_ASSERT(g.IO.RenderDrawListsFn == NULL);  // Call ImGui::RenderAPI() then pass ImGui::GetDrawData() yourself to your render function!
#endif
        }
        else
        {
    ...
    // But in order to allow the user to call NewFrame() multiple times without calling RenderAPI(), we are doing an explicit clear.
...
// This is normally called by RenderAPI(). You may want to call it directly if you want to avoid calling RenderAPI() but the gain will be very minimal.
void ImGui::EndFrame()
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
...
void ImGui::Render()
{
    ...
    // (Legacy) Call the RenderAPI callback function. The current prefer way is to let the user retrieve GetDrawData() and call the render function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    IM_ASSERT(g.FrameScopeActive);                  // Forgot to call ImGui::NewFrame()
    IM_ASSERT(g.FrameCountEnded != g.FrameCount);   // Called ImGui::RenderAPI() or ImGui::EndFrame() and haven't called ImGui::NewFrame() again yet
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            // RenderAPI resize grips (after their input handling so we don't have a frame of latency)
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    // RenderAPI default drop visuals
...
void ImGui::UpdatePlatformWindows()
{
    ...
    for (int i = 1; i < g.Viewports.Size; i++)
    {
        ...
        // Apply Position and Size (from ImGui to Platform/RenderAPI back-ends)
...
// The RenderAPI/Swap functions stored in ImGuiPlatformIO are merely here to allow for this helper to exist, but you can do it yourself:
//
...
static void ImGui::DockNodeUpdate(ImGuiDockNode* node)
{
    ...
    // RenderAPI outer borders last (after the tab bar)
    if (node->IsRootNode() && host_window)
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        static void NodeDrawList(ImGuiWindow* window, ImGuiViewportP* viewport, ImDrawList* draw_list, const char* label)
        {
            ...
            ImDrawList* fg_draw_list = viewport ? GetForegroundDrawList(viewport) : NULL; // RenderAPI additional visuals into the top-most draw list
