...
struct ImGuiPlatformIO;             // Multi-viewport support: interface for Platform/RenderAPI back-ends + viewports to render
...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new ImGui frame, you can submit any command from this point until RenderAPI()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the ImGui frame. automatically called by RenderAPI(), you likely don't need to call that yourself directly. If you don't need to render data (skipping rendering) you may call EndFrame() but you'll have wasted CPU already! If you don't need to render, better to not create any imgui windows and not call NewFrame() at all!
    ...
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after RenderAPI() and until the next call to NewFrame(). this is what you have to render. (Obsolete: this used to be passed to your io.RenderDrawListsFn() function.)
    ...
    IMGUI_API void          ShowFontSelector(const char* label);        // add font selector block (not a window), essentially a combo listing the m_loaded fonts.
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before RenderAPI(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasViewports  = 1 << 12   // Back-end RenderAPI supports multiple viewports.
};
...
struct ImGuiIO
{
    ...
    // Optional: Platform/RenderAPI back-end name (informational only! will be displayed in About Window) + User data for back-end/wrappers to store their own stuff.
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // [OBSOLETE since 1.60+] Rendering function, will be automatically called in RenderAPI(). Please call your rendering function yourself now!
    // You can obtain the ImDrawData* by calling ImGui::GetDrawData() after RenderAPI(). See example applications if you are unsure of how to implement this.
    ...
    int         MetricsRenderVertices;          // Vertices output during last call to RenderAPI()
    int         MetricsRenderIndices;           // Indices output during last call to RenderAPI() = number of triangles * 3
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // RenderAPI-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData
{
    bool            Valid;                  // Only valid after RenderAPI() is called and before the next NewFrame() is called.
...
//  - Optionally, call any of the AddFont*** functions. If you don't call any, the default font embedded in the code will be m_loaded for you.
...
struct ImFont
{
    ...
    ImFontAtlas*                ContainerAtlas;     // 4-8   // out //            // What we has been m_loaded into
...
// one for the Platform (~window handling), one for RenderAPI. Custom engine back-ends will often provide 
// both Platform and RenderAPI interfaces and so may not need to use all functions.
// Platform functions are typically called before their RenderAPI counterpart, 
...
struct ImGuiPlatformIO
{
    ...
    int     (*Platform_CreateVkSurface)(ImGuiViewport* vp, ImU64 vk_inst, const void* vk_allocators, ImU64* out_vk_surface); // (Optional) For RenderAPI to call into Platform code

    // (Optional) RenderAPI functions (e.g. DirectX, OpenGL3, Vulkan)
    ...
    void    (*Renderer_RenderWindow)(ImGuiViewport* vp, void* render_arg);  // (Optional) Clear targets, RenderAPI viewport->DrawData
    ...
    // List of viewports (the list is updated by calling ImGui::EndFrame or ImGui::RenderAPI)
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_NoRendererClear          = 1 << 5,   // Platform Window: RenderAPI doesn't need to clear the framebuffer ahead (because we will fill it entirely).
...
struct ImGuiViewport
{
    ...
    ImDrawData*         DrawData;               // The ImDrawData corresponding to this viewport. Valid after RenderAPI() and until the next call to NewFrame().
