...
namespace IW {
	...
	const iwm::vector3* mesh_factory::ico_verts = ico_vert_data;
		...
		size_t resolution)
	{
		...
		memcpy(mesh->Vertices, ico_verts, ico_vert_count  * sizeof(iwm::vector3));
		...
		size_t lonCount)
	{
		...
		float lonStep = 2 * iwm::PI / lonCount;
		float latStep  = iwm::PI / latCount;

		size_t vert = 0;
		for (size_t lat = 0; lat <= latCount; lat++) {
			for (size_t lon = 0; lon <= lonCount; lon++) {
				float y = sin(-iwm::PI / 2 + lat * latStep);
				...
				mesh->Vertices[vert] = iwm::vector3(x, y, z);
				mesh->Normals [vert] = iwm::vector3(x, y, z);
				mesh->Uvs     [vert] = iwm::vector2(u, v);
