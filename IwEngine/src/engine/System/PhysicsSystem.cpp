...
namespace IwEngine {
	...
	PhysicsSystem::PhysicsSystem(
		...
		: IwEngine::System<IW::Transform, IwPhysics::AABB3D>(space, renderer, "Physics")
	{}
		...
		for (auto entity : eca) {
			if (entity.Index == 1) {
				for (auto entity2 : eca) {
					...
					if (IwPhysics::AABB3D(transform1->Position + aabb1->Min, transform1->Position + aabb1->Max).Intersects(
						IwPhysics::AABB3D(transform2->Position + aabb2->Min, transform2->Position + aabb2->Max)))
					{
