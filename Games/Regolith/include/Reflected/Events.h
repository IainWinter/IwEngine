...
template<size_t L, class T, glm::qualifier Q>
inline const Class* GetClass(ClassTag<glm::vec<L, T, Q>>)
{
	static Class* c = nullptr;

	if (c == nullptr)
	{
		c = new Class("glm::vec<L, T, Q>", sizeof(glm::vec<L, T, Q>), 3);

		size_t offset = 0;

		c->fields[0] = { "x", GetType(TypeTag<T>()), offset, false };
		offset += sizeof(T);

		c->fields[1] = { "y", GetType(TypeTag<T>()), offset, false };
		offset += sizeof(T);

		c->fields[2] = { "z", GetType(TypeTag<T>()), offset, false };
	}
...
inline const Class* GetClass(ClassTag<glm::qua<T, Q>>)
{
	static Class* c = nullptr;

	if (c == nullptr)
	{
		c = new Class("glm::vec<L, T, Q>", sizeof(glm::qua<T, Q>), 4);
		...
		c->fields[1] = { "z", GetType(TypeTag<T>()), offset, false };
		offset += sizeof(T);

		c->fields[1] = { "w", GetType(TypeTag<T>()), offset, false };
		offset += sizeof(T);
	}
...
template<>
inline const Class* GetClass(ClassTag<EntityHandle>)
{
	static Class* c = nullptr;

	if (c == nullptr)
	{
		c = new Class("EntityHandle", sizeof(EntityHandle), 4);
...
inline const Class* GetClass(ClassTag<Transform>)
{
	...
	if (c == nullptr)
	{
		...
		c->fields[0] = { "Position", GetType(TypeTag<glm::vec<3, float, defaultp>>()), offset, false };
		offset += sizeof(vec<3, float, defaultp>);

		c->fields[1] = { "Scale", GetType(TypeTag<glm::vec<3, float, defaultp>>()), offset, false };
		offset += sizeof(vec<3, float, defaultp>);

		c->fields[2] = { "Rotation", GetType(TypeTag<glm::qua<float, defaultp>>()), offset, false };
	}
